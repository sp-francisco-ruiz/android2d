...
#include "engine/utils/lodepng.h"
...
namespace engine
{
    namespace platform
    {
        ...
        int GLESRenderer::GetWidth() const
        {
            return _width;
        }

        int GLESRenderer::GetHeight() const
        {
            return _height;
        }

        void GLESRenderer::SetWindow(ANativeWindow* window)
        {
            if(!window)
            {
        ...
        void GLESRenderer::DrawSprite(engine::graphics::SpriteNode& sprite)
        {
            auto texture = sprite.GetTexture();
            glBindTexture(GL_TEXTURE_2D, static_cast<GLuint>(texture->GetId()));
            glUniformMatrix4fv(modelUniform, 1, GL_FALSE, (GLfloat*)&sprite.GetWorldTrans());
        ...
        void GLESRenderer::SetAssetManager(AAssetManager* assetManager)
        {
            _assetManager = assetManager;
        }

        graphics::Texture* GLESRenderer::LoadTexture(const std::string& name)
        {
            auto texId = renderutils::CreateTexture(_assetManager, name.c_str());
            if(texId != 0xdead)
            {
                return new graphics::Texture(name, texId);
            }
            return nullptr;
        }
    }
